...
|---|---|---|---|
| **Extração** (`src`) | `sources/`, `internal/oshb`, `cmd/bvsrc` | fonte, manifest, packet, palavra, lemma, morfologia | → Editorial via arquivo (packet JSON) |
...
- `internal/oshb` (não `internal/osis`): o parser é específico do dialeto OSIS do
  OSHB (códigos morfológicos WLC); o NT (OpenGNT) é CSV, não OSIS.
